...
function migrateMasterLead(rec) {
  ...
  return {
    ...
    // Web Design Buyer Intent / Business Readiness / Phone Reachability (§ Buyer-Intent
    // upgrade) — additive diagnostics, never inputs to leadScore/leadTier above. Absent
    // on legacy/hard-rejected records, which is expected (never fabricated on migration).
    webDesignBuyerIntentScore: typeof r.webDesignBuyerIntentScore === 'number' ? r.webDesignBuyerIntentScore : null,
    webDesignBuyerIntentLevel: r.webDesignBuyerIntentLevel ?? null,
    intentDataSource: r.intentDataSource ?? null,
    nicheWebDesignDemand: r.nicheWebDesignDemand ?? null,
    localWebDesignDemand: r.localWebDesignDemand ?? null,
    commercialWebDesignSearchIntent: r.commercialWebDesignSearchIntent ?? null,
    businessReadinessScore: typeof r.businessReadinessScore === 'number' ? r.businessReadinessScore : null,
    businessReadinessBand: r.businessReadinessBand ?? null,
    phoneReachabilityScore: typeof r.phoneReachabilityScore === 'number' ? r.phoneReachabilityScore : null,
    phoneReachabilityType: r.phoneReachabilityType ?? null,
    gatekeeperRisk: typeof r.gatekeeperRisk === 'boolean' ? r.gatekeeperRisk : null,
    growthSignals: r.growthSignals ?? null,
    marketingActivitySignals: r.marketingActivitySignals ?? null,
    whyLookingForWebsite: r.whyLookingForWebsite ?? null,
...
export function addProcessedCandidates(scoredCandidates) {
  ...
  for (const c of Array.isArray(scoredCandidates) ? scoredCandidates : []) {
    ...
    next.push(migrateMasterLead({
      ...
      webDesignBuyerIntentScore: c.webDesignBuyerIntentScore,
      webDesignBuyerIntentLevel: c.webDesignBuyerIntentLevel,
      intentDataSource: c.intentDataSource,
      nicheWebDesignDemand: c.nicheWebDesignDemand,
      localWebDesignDemand: c.localWebDesignDemand,
      commercialWebDesignSearchIntent: c.commercialWebDesignSearchIntent,
      businessReadinessScore: c.businessReadinessScore,
      businessReadinessBand: c.businessReadinessBand,
      phoneReachabilityScore: c.phoneReachabilityScore,
      phoneReachabilityType: c.phoneReachabilityType,
      gatekeeperRisk: c.gatekeeperRisk,
      growthSignals: c.growthSignals,
      marketingActivitySignals: c.marketingActivitySignals,
      whyLookingForWebsite: c.whyLookingForWebsite,
